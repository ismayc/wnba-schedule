...
describe('condensed view strip', () => {
  ...
  it('expands to the full tab set and switches views from mid-page', async () => {
    ...
    await userEvent.click(within(tabs).getByRole('button', { name: '📊 Regular Season' }))
    ...
    expect(within(strip()).getByRole('button', { name: /📊 Regular Season/ })).toBeInTheDocument()
    const nav = screen.getByRole('navigation', { name: 'Views' })
    expect(within(nav).getByRole('button', { name: '📊 Regular Season' })).toHaveAttribute(
      'aria-current',
      'page'
    )
  })
  ...
  it('closes an open tab set when the real nav scrolls back into view', async () => {
    await mount()
    navAway()
    await userEvent.click(within(strip()).getByRole('button', { name: /📋 Schedule/ }))
